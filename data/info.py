...
def parse_tripinfo_file(file_path):
    ...
    for tripinfo in root.findall('tripinfo'):
        ...
        tripinfo_data['fuel'] = float(tripinfo_data['fuel_abs']) / float(tripinfo_data['routeLength'])
        tripinfo_data['CO2'] = float(tripinfo_data['CO2_abs']) / float(tripinfo_data['routeLength'])
...
for trip_id, data in tripinfo_dict.items():
    ...
    # print("Data:", data)
    total_fuel += float(data['fuel'])
    total_time += float(data['duration'])
    total_waitingTime += float(data['waitingTime'])
    total_co2 += float(data['CO2'])
    total_time_loss += float(data['timeLoss'])
print(total_time_loss / len(tripinfo_dict), total_fuel / len(tripinfo_dict) * 100)
print(total_time / len(tripinfo_dict), total_waitingTime / len(tripinfo_dict), total_co2 / len(tripinfo_dict))
