...
class ReplayBuffer:
    ...
    def add(self, s, a, a_logprob, r, s_, dw, done):
        self.s[self.count] = s
        self.a[self.count] = a
        ...
        self.count = (self.count + 1) % (self.size + 1)
        # print(self.count)
...
class Actor_Gaussian(nn.Module):
    def __init__(self, max_action, state_dim, hidden_width, action_dim, use_tanh, use_orthogonal_init):
...
class Critic(nn.Module):
    def __init__(self, state_dim, hidden_width, use_tanh, use_orthogonal_init):
...
class PPO_continuous():
    def __init__(
        ...
        if self.policy_dist == "Beta":
            ...
            self.actor = Actor_Gaussian(max_action, state_dim, hidden_width, action_dim, use_tanh=True,
                                        use_orthogonal_init=True)
        self.critic = Critic(state_dim, hidden_width, use_tanh=True, use_orthogonal_init=True)
    ...
    def add(self, s, a, a_logprob, r, s_, dw, done):
        self.replay_buffer.add(s, a, a_logprob, r, s_, dw, done)  # Add the data to the replay buffer

    def update(self, total_steps):
        s, a, a_logprob, r, s_, dw, done = self.replay_buffer.numpy_to_tensor()  # Get training data
        """
        ...
        for _ in range(self.K_epochs):
            ...
            for index in BatchSampler(SubsetRandomSampler(range(self.batch_size)), self.mini_batch_size, True):
                ...
                # print((a_logprob[index] - a_logprob_now).mean().item())
